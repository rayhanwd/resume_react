...
import { blog_json } from '../../utils/Database';
const Blog = () => {
  return (
            ...
            blog_json.map(data => 
              <div key={data.id} className="p-4 md:w-1/3">
                <div className="h-full border-2 border-gray-800 rounded-lg overflow-hidden">
                  ...
                  <div className="p-6">
                    <h2 className={`${data.color} tracking-widest text-xs title-font font-medium text-gray-500 mb-1`}>{data.category}</h2>
                    <h1 className="title-font text-lg font-medium text-white mb-3">{data.title}</h1>
                    <p className="leading-relaxed mb-3">{data.desc}</p>
                    <div className="flex items-center flex-wrap ">
                      <a className="text-indigo-400 inline-flex items-center md:mb-2 lg:mb-0 cursor-pointer">Learn More
